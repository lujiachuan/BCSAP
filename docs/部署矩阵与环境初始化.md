...
EPICS 命令定位是全自动的（界面上没有配置项）：执行服务依次从系统 `PATH`、`EPICS_BASE/bin/$EPICS_HOST_ARCH`、`EPICS_BASE/bin` 下的架构子目录以及打包服务 exe 同目录查找 `caget.exe` / `caput.exe`。控制电脑应先在普通命令提示符运行 `where caget`、`where caput`；修改系统环境变量后必须彻底退出并重启客户端，使它自动拉起的新服务继承最新环境。
