...
>
> 阶段注记：当前代码是 0.1.0 骨架。data_service 仍返回演示数据、PostgreSQL 未接入；instrument_service 的设备访问已统一走真实 EPICS 通道访问（本机没有 IOC 时健康检查会如实报未连接，联调可起仓库 `sim/` 下的模拟 IOC）。本文先给出**目标拓扑的完整初始化与验收方式**，凡尚未实现的点均标注 `[待接入]`，避免照文档操作时产生误判。
...
真实 EPICS 接入：设备访问统一走真实 EPICS Channel Access，**没有模拟/真实模式开关**。在“系统设置 → PV 映射”里编辑“业务信号 → PV”表（增删改行、填写现场 IOC 上的真实 PV 名），保存后由执行服务校验并立即生效。映射是执行服务的**受控配置**（按架构文档 6.2），落盘在 `%APPDATA%\SpectrumPlatform\pv_mapping.json`（可用环境变量 `SPECTRUM_PV_MAPPING` 改路径），客户端只通过 `/control/v1/pv-mapping` 读写、不直接访问 IOC。

CA 库定位是全自动的（界面上没有配置项）：执行服务按 `SPECTRUM_CA_LIB_DIR` → `EPICS_BASE/bin/$EPICS_HOST_ARCH` → `EPICS_BASE/bin` → 同盘 `CA-*-windows-*` 目录的顺序尝试加载 `ca.dll`，并把每个候选目录的失败原因写进健康明细。注意 EPICS base 的 mingw 构建依赖 `libgcc_s_seh-1.dll` / `libstdc++-6.dll`，这两个 DLL 缺失时无法加载，此时会自动落到官方 MSVC 构建的 CA 分发；只有自动发现全失败才需要显式设 `SPECTRUM_CA_LIB_DIR`，而**显式指定时只认该目录、不再回退**。

本机没有 IOC 时，健康检查会如实报 `unavailable`（逐项 `PV 未连接`），扫谱与自动调束入口保持禁用——这是预期行为，不会退化成模拟。开发与联调请起仓库 `sim/` 下的本地模拟 IOC（见 `sim/README.md`）。
