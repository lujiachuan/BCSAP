...
public:
	...
	char					szName[MAX_STRING_SIZE];				// PV name as null-terminated string
	...
	calabItem(LStrHandle name, sStringArrayHdl fieldNames = 0x0) {
		...
		if ((*name)->cnt < MAX_STRING_SIZE - 1) {
			...
			NumericArrayResize(uB, 1, (UHandle*)&this->name, MAX_STRING_SIZE - 1);
			memcpy((*this->name)->str, (*name)->str, MAX_STRING_SIZE - 1);
			(*this->name)->cnt = MAX_STRING_SIZE - 1;
			memcpy(szName, (*name)->str, MAX_STRING_SIZE - 1);
			szName[MAX_STRING_SIZE - 1] = 0x0;
			CaLabDbgPrintf("%s was cropped to %d characters (max EPICS name size)", szName, MAX_STRING_SIZE - 1);
		}
		...
		if (fieldNames) {
			char szFieldName[MAX_STRING_SIZE];
