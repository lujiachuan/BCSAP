...
// Version     : 1.7.0.0
// Copyright   : HZB
// Description : Library for remote control of the SoftIoc from the EPICS BASE
//==================================================================================================
...
//EXTERNC void disconnectAll(void);
...
static bool ProcessByName(const char* wcProcessFileName, bool kill) {
    HANDLE hProcessSnap;
    HANDLE hProcess;
    PROCESSENTRY32 pe32;
    ...
    do {
        if (!strcmp(pe32.szExeFile, wcProcessFileName)) {
            if (kill) {
...
EXTERNC EXPORT int softIoc(LVBoolean *ForceOneIOC) {
    if (*ForceOneIOC) {
...
//    PVList: list of PVs
EXTERNC EXPORT void addPVList(sStringArrayHdl* PVList) {
    if (!((***PVList).elt)[0])
        return;
