//
// link this file NOT with liblv.so
//
...
#include <windows.h>
#include <stdint.h>
#define _STDINT_H_ 1 // needed for correct subordinated includes in extcode.h
#endif
#include <extcode.h>
#include <atomic>
#include <chrono>
...
#if !defined firstEpicsAlarmSev // maintain compatibility with old EPICS Base versions
#include <alarm.h>
#endif
#include <cadef.h>
#include <dbDefs.h>
...
#endif
#define CALAB_VERSION       "1.7.0.1"
#define ERROR_OFFSET        7000           // User defined error codes of LabVIEW start at this number
#define MAX_ERROR_SIZE		255
#define MAX_NAME_SIZE 		255            // PVNAME_STRINGSZ (61) was to short for some internal processing functions
...
#ifdef WIN
HMODULE						libRef = 0x0;         // additional handle for this library (prevent unload of lib)
#else
void*						libRef = 0x0;
#endif
struct ca_client_context*	pcac = 0x0;            // EPICS context
...
class calabItem {														// internal data object
public:
	...
	bool						initConnect;							// indicator for Channel Access connection

	calabItem(LStrHandle name, sStringArrayHdl fieldNames = 0x0) {
		void* fieldSeparator = 0x0;
		initConnect = false;
		hasValue = false;
		isConnected = false;
		isPassive = false;
		...
		// error case for invalid PV name
		if (!name) {
			DbgTime(); CaLabDbgPrintf("ERROR: Empty PV name found! (After error handling, please restart LabVIEW cpl.)");
			NumericArrayResize(uB, 1, (UHandle*)&name, 1);
			(*name)->cnt = 1;
			(*name)->str[0] = '?';
		}
		// check and process PV name with attached field name
		fieldSeparator = memchr((*name)->str, '.', (*name)->cnt);
		if ((*name)->cnt > PVNAME_SZ && !fieldSeparator) {
			NumericArrayResize(uB, 1, (UHandle*)&this->name, PVNAME_SZ);
			memcpy((*this->name)->str, (*name)->str, PVNAME_SZ);
			(*this->name)->cnt = PVNAME_SZ;
			memcpy(szName, (*name)->str, PVNAME_SZ);
			szName[PVNAME_SZ] = 0x0;
			CaLabDbgPrintf("PV name was cropped to %s (max EPICS name size is %d)", szName, PVNAME_SZ);
		}
		else {
		...
		if (strchr(szName, ' ') || strchr(szName, '\t')) {
			...
			if (!strlen(szName)) {
				...
				szName[1] = 0x0;
				(*name)->str[0] = '?';
				DbgTime(); CaLabDbgPrintf("ERROR: PV name contains white space(s) only (After error handling, please restart LabVIEW cpl.)");
			}
		...
		if (fieldNames) {
			...
			for (uInt32 i = 0; i < (*fieldNames)->dimSize; i++) {
				// error case for invalid field name
				if (!(*fieldNames)->elt[i]) {
					DbgTime(); CaLabDbgPrintf("ERROR: Empty field name found! (After error handling, please restart LabVIEW cpl.)");
					NumericArrayResize(uB, 1, (UHandle*)&(*FieldNameArray)->elt[i], 1);
					(*(*FieldNameArray)->elt[i])->cnt = 1;
					(*(*FieldNameArray)->elt[i])->str[0] = '?';
				}
				else {
				...
				if (strchr(szFieldName, ' ') || strchr(szFieldName, '\t')) {
					...
					if (!strlen(szFieldName)) {
						szFieldName[0] = '?';
						szFieldName[1] = 0x0;
						(*(*fieldNames)->elt[i])->str[0] = '?';
						DbgTime(); CaLabDbgPrintf("ERROR: field name contains white space(s) only (After error handling, please restart LabVIEW cpl.)");
					}
...
#ifdef _DEBUG
		if (szName) {
			DbgTime(); CaLabDbgPrintf("Added %s to CA Lab", szName);
		}
#endif
	}
	...
	~calabItem() {
		...
		// Since CA Lab version 1.7.0.0 and higher works as a service, we let the LabVIEW memory manager clean up the variables allocated in LabVIEW.
		/*
		MgErr err = noErr;
	...
	void lock() {
		...
		diff = std::chrono::high_resolution_clock::now() - lockTimer;
		if (diff.count() > 5) {
			DbgTime(); CaLabDbgPrintf("%s has delayed mutex", szName);
		}
#endif
	}
	...
	void itemValueChanged(evargs args) {
		try {
			...
			if (stringValueArray && DSCheckHandle(stringValueArray) != noErr) {
				unlock();
				return;
			}
	...
	//    Timeout:            EPICS event timeout in seconds (integer since CA Lab version 1.7.0.0)
	//    Synchronous:        true = callback will be used (no interrupt of motor records)
	void put(void* ValueArray2D, uInt32 DataType, uInt32 Row, uInt32 ValuesPerSet, sError* Error, uint8_t Timeout, bool synchronious) {
...
public:
	...
	~calabItemList() {
		// we no longer have to wait for the other tasks since CA Lab version 1.7.0.0
		/*
		uInt32 timeout = 1000;
		...
		*/
		...
		// see https://epics.anl.gov/tech-talk/2020/msg00963.php and sub threads
		//ca_context_destroy();
		//caLabUnload();
	}
	...
	calabItem* add(LStrHandle name, sStringArrayHdl FieldNameArray = 0x0) {
		...
		calabItem* fieldItem;
		// invalid case for empty PV name
		if (!name) {
			DbgTime(); CaLabDbgPrintf("ERROR: Empty PV name found! (After error handling, please restart LabVIEW cpl.)");
			NumericArrayResize(uB, 1, (UHandle*)&name, 1);
			(*name)->cnt = 1;
		...
		// invalid name
		if (name && ((*name)->cnt <= 1 || (*name)->str[0] == ' ' || (*name)->str[0] == '\t')) {
			NumericArrayResize(uB, 1, (UHandle*)&name, 1);
			(*name)->cnt = 1;
			(*name)->str[0] = '?';
#ifdef _DEBUG
			DbgTime(); CaLabDbgPrintf("ERROR: invalid PV name found!");
#endif
		}
		while (currentItem) {
...
EXTERNC EXPORT MgErr reserved(InstanceDataPtr * instanceState) {
	return 0;
}

// callback of LV when any caLab-VI is unloaded
//    instanceState: undocumented pointer
EXTERNC EXPORT MgErr unreserved(InstanceDataPtr * instanceState) {
	return 0;
}

// callback of LV when any caLab-VI is aborted
//    instanceState: undocumented pointer
EXTERNC EXPORT MgErr aborted(InstanceDataPtr * instanceState) {
...
EXTERNC EXPORT void getValue(sStringArrayHdl * PvNameArray, sStringArrayHdl * FieldNameArray, sLongArrayHdl * PvIndexArray, uint8_t Timeout, sResultArrayHdl * ResultArray, sStringArrayHdl * FirstStringValue, sDoubleArrayHdl * FirstDoubleValue, sDoubleArray2DHdl * DoubleValueArray, LVBoolean * CommunicationStatus, LVBoolean * FirstCall, LVBoolean * NoMDEL = 0, LVBoolean * IsInitialized = 0) {
	if(!libRef) return; // without valid library reference no access to the function
...
EXTERNC EXPORT void addEvent(LVUserEventRef * RefNum, sResult * ResultPtr) {
	if(!libRef) return; // without valid library reference no access to the function
	// Don't enter if library terminates
	if (stopped)
		return;
...
//        6 => Quad signed integer              => long      => dbr_long_t
EXTERNC EXPORT void putValue(sStringArrayHdl * PvNameArray, sLongArrayHdl * PvIndexArray, sStringArray2DHdl * StringValueArray2D, sDoubleArray2DHdl * DoubleValueArray2D, sLongArray2DHdl * LongValueArray2D, uInt32 DataType, uint8_t Timeout, LVBoolean * Synchronous, sErrorArrayHdl * ErrorArray, LVBoolean * Status, LVBoolean * FirstCall) {
	if(!libRef) return; // without valid library reference no access to the function
	try {
		// Don't enter if library terminates
		if (stopped)
...
//   FirstCall:             indicator for first call
EXTERNC EXPORT void info(sStringArray2DHdl * InfoStringArray2D, sResultArrayHdl * ResultArray, LVBoolean * FirstCall) {
	if(!libRef) return; // without valid library reference no access to the function
	try {
		// Don't enter if library terminates
		if (stopped)
...
EXTERNC EXPORT void disconnectPVs(sStringArrayHdl * PvNameArray, bool All) {
	if(!libRef) return; // without valid library reference no access to the function
	try {
		// Don't enter if library terminates
		if (stopped)
...
EXTERNC EXPORT uInt32 getCounter() {
...
#else
	libRef = dlopen("libCaLab.so", RTLD_LAZY | RTLD_NODELETE); // RTLD_NODELETE to prevent unload lib
	if (!libRef) {
		DbgTime(); CaLabDbgPrintf("Error: libCaLab.so not found: %s", dlerror());
		return;
	}
#endif
